      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="flex flex-col items-center text-center">
          ...
          <div className="flex items-center gap-2 mt-2">
            <StarRating rating={Math.round(user.rating)} size="sm" />
            <span className="text-brand text-sm font-semibold">{user.rating}</span>
          </div>
          <span className="mt-2 bg-brand text-[#0E0E0E] px-3 py-0.5 rounded-full text-xs font-bold uppercase">{user.role}</span>
          <p className="text-text-muted text-xs mt-3">{user.jobsCompleted} jobs completed</p>
        </Card>
        <Card hover={false} className="lg:col-span-2">
          ...
          <form className="space-y-4">
            ...
            <div>
              <label className="block text-text-primary mb-1.5 text-sm font-semibold">Skills</label>
              <div className="flex flex-wrap gap-2">
                {user.skills.map((s) => (
                  <span key={s} className="bg-brand/10 text-brand px-3 py-1 rounded-full text-xs font-semibold">{s}</span>
                ))}
              </div>
            </div>
